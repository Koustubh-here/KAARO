...
export async function runAgent({ user, business, userText }) {
  ...
  // Upsert identity memory for better recall
  try {
  ...
  // Direct answers for identity/business questions
  const lowerQ = (userText || '').toLowerCase();
  if (/(who am i|what is my name)\??/.test(lowerQ)) {
    const reply = (user?.user_metadata?.full_name || user?.email || user?.id) ? `You are ${user?.user_metadata?.full_name || user?.email || user?.id}.` : 'I know you are signed in, but I do not have your name on file.';
    await appendMessage(conversationId, 'assistant', reply, null);
    return { reply };
  }
  if (/(what is my business|what's my business|my shop name)\??/.test(lowerQ)) {
    const reply = ensuredBusiness?.name ? `Your business is ${ensuredBusiness.name}.` : 'I could not find your business yet.';
    await appendMessage(conversationId, 'assistant', reply, null);
    return { reply };
  }
